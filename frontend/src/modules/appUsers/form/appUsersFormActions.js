...
const actions = {
  ...
  doCreate: (values) => async (dispatch) => {
    try {
      ...
      await AppUsersService.create(values);

      dispatch({
        type: actions.CREATE_SUCCESS,
      });

      Message.success(i18n('entities.appUsers.create.success'));
  ...
  doUpdate: (id, values) => async (
    ...
    try {
      ...
      await AppUsersService.update(id, values);

      dispatch({
        type: actions.UPDATE_SUCCESS,
      });

      Message.success(i18n('entities.appUsers.update.success'));
