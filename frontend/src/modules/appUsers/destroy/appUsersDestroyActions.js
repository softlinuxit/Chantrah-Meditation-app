import listActions from 'modules/appUsers/list/appUsersListActions';
...
const actions = {
  ...
  doDestroy: (id) => async (dispatch) => {
    try {
      ...
      await AppUsersService.destroyAll([id]);

      dispatch({
        type: actions.DESTROY_SUCCESS,
      });

      Message.success(i18n('entities.appUsers.destroy.success'));
  ...
  doDestroyAll: (ids) => async (dispatch) => {
    try {
      ...
      await AppUsersService.destroyAll(ids);
      ...
      Message.success(
        i18n('entities.appUsers.destroyAll.success'),
      );
